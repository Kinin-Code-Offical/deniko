...
export async function studentRoutes(fastify: FastifyInstance) {
    ...
    fastify.get('/:id', async (request, reply) => {
        ...
        if (!relation) return reply.code(404).send({ error: 'student_not_found' });
    ...
    fastify.get('/invite/:token', async (request, reply) => {
        ...
        if (!student) return reply.code(404).send({ error: 'invite_not_found' });
        if (student.userId) return reply.code(400).send({ error: 'already_claimed' });
    ...
    fastify.post('/claim', async (request, reply) => {
        ...
        if (existingProfile) return reply.code(400).send({ error: 'user_already_has_profile' });
    ...
    fastify.post('/:id/invite/toggle', async (request, reply) => {
        ...
        let data: Prisma.StudentProfileUpdateInput = {};
        if (enable) {
            data = {
