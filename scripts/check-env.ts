...
const IGNORE_DIRS = ['node_modules', '.next', 'dist', 'build', 'coverage', '.git', '.vscode', 'scripts'];
...
if (isMainModule) {
    ...
    files.forEach(file => {
        ...
        if (file.endsWith('next.config.ts') || file.endsWith('next.config.js') || file.endsWith('next.config.mjs')) return; // Config files often need process.env
    ...
    console.log('\n--------------------------------------------------');
    if (errorCount > 0) {
        console.error(`❌ Found \x1b[31m${errorCount}\x1b[0m occurrences of 'process.env' in ${fileCount} files.`);
        console.log('💡 Tip: Use env from @/lib/env or // ignore-env-check to bypass.');
        process.exit(1);
    } else {
