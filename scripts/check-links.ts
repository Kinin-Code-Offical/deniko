...
const IGNORED_PREFIXES = ['http', 'https', 'mailto:', 'tel:', '#', 'javascript:'];
...
function checkRouteExists(route: string): boolean {
    ...
    if (parts.length > 0 && parts[0].length === 2) {
        ...
        if (exists(path.join(APP_DIR, '[lang]', ...partsWithoutLocale))) return true;
    }
...
async function main() {
    ...
    files.forEach((file) => {
        ...
        links.forEach((link) => {
            ...
            if (link.includes('${') || link.includes('[') || link.includes('+')) return;
            ...
            if (!checkRouteExists(link)) {
                ...
                console.error(`❌ Broken link found in ${path.relative(process.cwd(), file)}: ${link}`);
                brokenLinksCount++;
            }
        });
    });
