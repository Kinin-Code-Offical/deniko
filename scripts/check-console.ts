...
const IGNORE_DIRS = ['node_modules', '.next', 'dist', 'build', 'coverage', '.git', '.vscode', 'scripts', 'debug-tests'];
...
if (isMainModule) {
    ...
    console.log('\n--------------------------------------------------');
    if (errorCount > 0) {
        console.error(`❌ Found \x1b[31m${errorCount}\x1b[0m occurrences of 'console.*' in ${fileCount} files.`);
        console.log('💡 Tip: Use logger from @/lib/logger or // ignore-console-check to bypass.');
        process.exit(1);
    } else {
